...
function Sidebar() {
  ...
  // 👈 Buat fungsi handleLogout untuk membersihkan sesi dan pindah halaman
  const handleLogout = () => {
    logout(); // Memanggil fungsi logout dari context (menghapus token, dll)
    navigate("/login"); // Mengarahkan user kembali ke halaman login
  };
    ...
    <aside className="hidden lg:flex flex-col w-[280px] bg-white dark:bg-slate-900 border-r border-gray-100 dark:border-slate-800 min-h-screen px-6 py-8 transition-all">
      ...
      <div className="mt-auto">
        ...
        <button 
          ...
          className="w-full flex items-center gap-4 px-5 py-4 rounded-2xl text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-all font-medium"
        >
          <LogOut size={22} />
          <span>
            {t.logout}
